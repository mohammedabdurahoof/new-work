"use client"
import React, { useState } from 'react'
import FoldNav from './FoldNav';
import UnFoldNav from './UnFoldNav';
import ModeBaisedImage from './ModeBaisedImage';

interface Props {
    open: boolean;
    setOpen: React.Dispatch<React.SetStateAction<boolean>>;
}

function SideBar({ open, setOpen }: Props) {


    return (
        <div className="p1nc1a90" id={open ? "page-unfold-sidebar" : "page-fold-sidebar"} style={open ? { width: '260px', transform: 'none' } : { width: '260px', transform: 'translateX(-196px) translateZ(0px)' }}>
            <div className={`pc035jc ${!open && "sidebar-header-fold"}`}>
                <div className="header-sidebar-inner">
                    <button className="fold-btn" onClick={() => setOpen(!open)}>
                        <svg className="s1ff97qc icon">
                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Tighten"></use>
                        </svg></button>
                    {open && <a href="/" keep-scroll-position="true" className="logo-pc is-active">
                        {/* <img alt="logo"
                        className="logo-com" src="/images/logo.33451179.png" /> */}
                        <ModeBaisedImage className='logo-com' darkImage='/images/logo.33451179.png' lightImage='/images/logo_w.png' />
                        </a>}
                </div>
            </div>
            <div className="pc-sidebar-inner">
                {
                    open ? <UnFoldNav /> : <FoldNav />
                }
                {/* <div className="u1hrvna7 hidden-scroll-y unfold-scroll-wrap">
                    <div className="pk7420m bonus-link-enter">
                        <div className="nav-item-wrap"><a href="/bonus" keep-scroll-position="true" className="nav-item">
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Gift"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Bonus</span></div>
                        </a></div>
                    </div>
                    <div className="bryd93e bonus-list-wrap">
                        <div className="bl-l bl-l-2">
                            <div className="bonus-item li-quests" style={{ position: 'relative' }}><button
                                className="qvvvzmo bonus-item-btn">
                                <div className="img-wrap"><img className="img-bg" src="/images/quests.7ae7831a.png" alt="quests" />
                                </div>
                            </button>
                                <div className="bonus-text">
                                    <p className="t ttu task">Quest</p>
                                </div>
                            </div>
                            <div className="bonus-item li-spin" style={{ position: 'relative' }}><button className="e1vxjz6v bonus-item-btn">
                                <div className="enter-animate active">
                                    <div className="spin-anim"><img className="img-spinbg" src="/images/spin_bg.c87bd0f4.png"
                                        alt="" /><img className="img-turntable" src="/images/turntable.b21b0d0a.png"
                                            alt="" /><img className="img-pointer" src="/images/pointer.fe72597d.png" alt="" /></div>
                                </div>
                            </button>
                                <div className="bonus-text">
                                    <p className="t ttu spin">Spin</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="activity-box">
                        <div className="act-item-left"><img alt="act-friend" src="/images/activity_icon.b7616940.png" /></div>
                        <div className="act-item-right">
                            <div>BC Anniversary</div>
                            <div className="char">Exploration &amp; Riches Await!</div>
                        </div>
                    </div>
                    <div className={casino ? "cvb7yhf nav-item-wrap casino-open" : "cvb7yhf nav-item-wrap"}>
                        <div className={casino ? "toggle-tigger nav-item open-tigger" : "toggle-tigger nav-item"} onClick={() => setCasino(!casino)}>
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Casino"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Casino</span><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Arrow"></use>
                            </svg></div>
                        </div>
                        <div className="v1qcofba" style={casino ? { height: 'auto' } : { height: '0px' }}>
                            <div className="casino-sub-navs"><a href="/gamelist/picks-for-you" keep-scroll-position="true"
                                className="nav-item">
                                <div className="nav-item-left"><svg className="s1ff97qc icon">
                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_PicksForYou"></use>
                                </svg></div>
                                <div className="nav-item-right"><span>Picks For You</span></div>
                            </a><a keep-scroll-position="true" className="nav-item no-link">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Favorites"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Favorites</span></div>
                                </a><a keep-scroll-position="true" className="nav-item no-link">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Recent"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Recent</span></div>
                                </a>


                                <div className="game-nav-wrap"><a href="/casino?tab=brand" keep-scroll-position="true"
                                    className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_classNameicDice"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>BC Originals</span><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Arrow"></use>
                                    </svg></div>
                                </a>
                                    <div className="place"></div>
                                    <div className="game-list-nav-wrap">
                                        <div className="game-list-shadow-inner">
                                            <div className="game-list-wrap hidden-scroll-y"><a href="/game/crash"
                                                keep-scroll-position="true" className="game-item">
                                                <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Crash"></use>
                                                </svg><img src="/images/Crash.ec1942f7.png" className="game-icon-img"
                                                    alt="" /></div><span className="game-name">Crash</span>
                                            </a><a href="/game/fastparity" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Parity">
                                                        </use>
                                                    </svg><img src="/images/Parity.f8c5ee6f.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">FastParity</span>
                                                </a><a href="/game/classNameic-dice" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_classNameicDice">
                                                        </use>
                                                    </svg><img src="/images/classNameicDice.246bc06a.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">classNameic Dice</span>
                                                </a><a href="/game/limbo" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Limbo"></use>
                                                    </svg><img src="/images/Limbo.011b8b65.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Limbo</span>
                                                </a><a href="/game/plinko" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Plinko">
                                                        </use>
                                                    </svg><img src="/images/Plinko.408dd155.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Plinko</span>
                                                </a><a href="/game/hash-dice" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_HashDice">
                                                        </use>
                                                    </svg><img src="/images/HashDice.445aff9f.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Hash dice</span>
                                                </a><a href="/game/ultimate-dice" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use
                                                            xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_UltimateDice">
                                                        </use>
                                                    </svg><img src="/images/UltimateDice.e3fb51c3.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Ultimate Dice</span>
                                                </a><a href="/game/keno" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Keno"></use>
                                                    </svg><img src="/images/KenoSingle.6244c89e.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Keno</span>
                                                </a><a href="/game/wheel" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Wheel"></use>
                                                    </svg><img src="/images/Wheel.4ec42835.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Wheel</span>
                                                </a><a href="/game/twist" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Twist"></use>
                                                    </svg><img src="/images/Twist.aeed2ac6.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Twist</span>
                                                </a><a href="/game/mines" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Mines"></use>
                                                    </svg><img src="/images/Mines.41c036c1.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Mines</span>
                                                </a><a href="/game/cave" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Cave"></use>
                                                    </svg><img src="/images/Cave.89f3fbd0.png" className="game-icon-img" alt="" />
                                                    </div><span className="game-name">Cave</span>
                                                </a><a href="/game/tower-legend" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_TowerLegend">
                                                        </use>
                                                    </svg><img src="/images/Tower.3d93671f.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Tower Legend</span>
                                                </a><a href="/game/coinflip" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Coinflip">
                                                        </use>
                                                    </svg><img src="/images/CoinFlip.340b3677.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Coinflip</span>
                                                </a><a href="/game/hilo" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Hilo"></use>
                                                    </svg><img src="/images/HiLo.86a19c7b.png" className="game-icon-img" alt="" />
                                                    </div><span className="game-name">Hilo</span>
                                                </a><a href="/game/roulette" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Roulette">
                                                        </use>
                                                    </svg><img src="/images/RouletteSingle.48e1c85a.png"
                                                        className="game-icon-img" alt="" /></div><span
                                                            className="game-name">Roulettes</span>
                                                </a><a href="/game/ring-of-fortune" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use
                                                            xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_RingOfFortune">
                                                        </use>
                                                    </svg><img src="/images/Coloring.9267a41c.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Ring of Fortune</span>
                                                </a><a href="/game/oriental-beauties" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use
                                                            xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_OrientalBeauties">
                                                        </use>
                                                    </svg><img src="/images/OrientalBeauties.2fbcd1a8.png"
                                                        className="game-icon-img" alt="" /></div><span
                                                            className="game-name">Beauties</span>
                                                </a><a href="/game/sword" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Sword"></use>
                                                    </svg><img src="/images/Swords.1712c95b.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Sword</span>
                                                </a><a href="/game/video-poker" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_VideoPoker">
                                                        </use>
                                                    </svg><img src="/images/VideoPoker.43ba50ae.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Video Poker</span>
                                                </a><a href="/game/baccarat" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Baccarat">
                                                        </use>
                                                    </svg><img src="/images/BaccaratSingle.857259c1.png"
                                                        className="game-icon-img" alt="" /></div><span
                                                            className="game-name">Baccarat</span>
                                                </a><a href="/game/blackjack" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Blackjack">
                                                        </use>
                                                    </svg><img src="/images/BlackJack.7f868444.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Blackjack</span>
                                                </a><a href="/game/double" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Double">
                                                        </use>
                                                    </svg><img src="/images/Double.ecdb8a52.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Double</span>
                                                </a><a href="/game/keno-multiplayer" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use
                                                            xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_KenoMultiplayer">
                                                        </use>
                                                    </svg><img src="/images/Keno.4596331e.png" className="game-icon-img" alt="" />
                                                    </div><span className="game-name">Keno multiplayer</span>
                                                </a><a href="/game/roulette-multiplayer" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use
                                                            xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_RouletteMultiplayer">
                                                        </use>
                                                    </svg><img src="/images/Roulette.6b277c23.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Roulette Multiplayer</span>
                                                </a><a href="/game/evo21:blackjack:reyd5odmlwpfzjdw" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_BlackJackA">
                                                        </use>
                                                    </svg><img src="/images/BlackJackA.f17772ea.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">BC Blackjack A</span>
                                                </a><a href="/game/evo21:blackjack:reyezjtolwpf37px" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_BlackJackB">
                                                        </use>
                                                    </svg><img src="/images/BlackJackB.39850ec8.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">BC Blackjack B</span>
                                                </a><a href="/game/baccarat-multiplayer" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="s1ff97qc icon game-icon-svg">
                                                        <use
                                                            xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_BaccaratMultiplayer">
                                                        </use>
                                                    </svg><img src="/images/Baccarat.47e9d8f2.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">Baccarat multiplayer</span>
                                                </a><a href="/game/evo21:blackjack:rgea7vnq54ehg2fs" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="add-icon game-icon-svg icon"
                                                        viewBox="0 0 1024 1024">
                                                        <path
                                                            d="m654.997 123.938.491.29 346.281 206.985a46.645 46.645 0 0 1 20.823 28.354 46.636 46.636 0 0 1-4.92 34.812l-.29.491-292.225 488.817a46.648 46.648 0 0 1-28.641 21.296 46.636 46.636 0 0 1-34.812-4.92l-.491-.29-345.778-206.699a46.648 46.648 0 0 1-21.296-28.641 46.636 46.636 0 0 1 4.92-34.812l.29-.491 292.197-488.817a46.648 46.648 0 0 1 28.641-21.296 46.636 46.636 0 0 1 34.812 4.92zm-211.245 16.917a46.635 46.635 0 0 1 24.09 25.649l.195.518 17.737 47.717-256.096 447.22a46.577 46.577 0 0 0-4.527 35.389 46.69 46.69 0 0 0 21.36 27.966l.478.278-.085-.029 191.467 109.641-177.374 65.853a46.613 46.613 0 0 1-35.646-1.338 46.682 46.682 0 0 1-24.062-25.649l-.195-.518L2.908 339.637a46.613 46.613 0 0 1 1.338-35.646 46.682 46.682 0 0 1 25.649-24.062l.518-.195 377.666-140.191a46.553 46.553 0 0 1 35.674 1.309zM202.305 270.236l-48.425 18.589-28.621 93.517-2.243.861-27.611-71.932-41.115 15.783 65.299 170.11 41.115-15.783-17.695-46.099 8.475-27.076 68.028 50.604 49.172-18.875-97.539-70.629 31.161-99.066zm492.712-31.351-74.176 128.477c-3.169 5.348-6.572 9.392-10.21 12.131s-7.471 4.175-11.5 4.308-8.191-1.039-12.486-3.519c-4.113-2.375-7.163-5.264-9.149-8.669s-2.842-7.266-2.567-11.583c.275-4.318 1.701-8.918 4.278-13.801l-44.732-25.826c-7.743 13.275-11.279 25.789-10.608 37.548s4.57 22.398 11.695 31.916c7.127 9.518 16.497 17.63 28.111 24.335 13.127 7.578 26.126 11.777 39 12.597s24.747-1.62 35.619-7.32c10.872-5.699 19.813-14.551 26.823-26.553l74.176-128.477-44.277-25.564z">
                                                        </path>
                                                    </svg><img src="/images/SpeedBlackjack.3a7b41d4.png"
                                                        className="game-icon-img" alt="" /></div><span className="game-name">BCGAME
                                                            Speed Blackjack</span>
                                                </a><a href="/game/evo21:blackjack:rgdzue7v4xaga5s5" keep-scroll-position="true"
                                                    className="game-item">
                                                    <div className="game-icon"><svg className="add-icon game-icon-svg icon"
                                                        viewBox="0 0 1024 1024">
                                                        <path
                                                            d="m636.234 58.055.517.304 363.888 217.51a49.013 49.013 0 0 1 21.882 29.797 48.988 48.988 0 0 1-5.17 36.582l-.304.517-164.323 274.869c-14.391-3.868-29.521-5.931-45.133-5.931l-551.985.001a49.053 49.053 0 0 1 6.591-22.252l.304-.517L569.555 75.264a49.01 49.01 0 0 1 30.098-22.379 48.988 48.988 0 0 1 36.582 5.17zM376.309 184.581a39.548 39.548 0 0 1 20.406 21.69l.189.498 15.042 40.465-208.71 364.469h-81.904c-7.577 0-15.042.486-22.362 1.429L2.467 353.154a39.525 39.525 0 0 1 1.134-30.228 39.585 39.585 0 0 1 21.69-20.382l.498-.189L346.057 183.47a39.478 39.478 0 0 1 30.252 1.111zM171.558 294.298l-41.065 15.764-24.271 79.304-1.902.73-23.416-60.999-34.867 13.384 55.375 144.256 34.867-13.384-15.006-39.093 7.187-22.963 57.689 42.913 41.699-16.007-82.715-59.895 26.424-84.01zm506.73-115.452L600.34 313.855c-3.33 5.62-6.906 9.87-10.728 12.748s-7.851 4.387-12.085 4.527c-4.234.14-8.607-1.092-13.12-3.697-4.322-2.496-7.527-5.532-9.614-9.111s-2.986-7.635-2.697-12.172c.278-4.369 1.678-9.013 4.2-13.934l.296-.569-47.006-27.139c-8.137 13.948-11.852 27.1-11.147 39.457s4.802 23.536 12.291 33.539c7.489 10.002 17.335 18.526 29.539 25.573 13.794 7.964 27.454 12.376 40.983 13.237s26.006-1.702 37.431-7.692c11.221-5.882 20.484-14.96 27.79-27.23l.397-.672 77.948-135.009-46.529-26.864zm126.758 471.19c89.072 0 161.279 72.207 161.279 161.279s-72.207 161.279-161.279 161.279H166.883c-89.072 0-161.279-72.207-161.279-161.279s72.207-161.279 161.279-161.279h638.163zm-470.118 68.251h-50.447l62.922 186.055h60.008l62.922-186.055h-50.447l-41.704 136.271h-1.548L334.93 718.287zm189.919 0h-45.074v186.055h45.074V718.287zm94.579 0H542.39v186.055h45.074v-57.779h30.595c14.265 0 26.589-2.634 36.97-7.903s18.379-12.704 23.994-22.303c5.615-9.599 8.423-20.819 8.423-33.659 0-12.779-2.762-23.999-8.286-33.659s-13.371-17.2-23.539-22.621c-10.168-5.42-22.234-8.131-36.196-8.131zm-9.743 36.248c6.738 0 12.415 1.151 17.028 3.452s8.104 5.556 10.472 9.766c2.368 4.21 3.552 9.191 3.552 14.944 0 5.693-1.184 10.674-3.552 14.944s-5.843 7.586-10.426 9.948-10.275 3.543-17.073 3.543h-22.218v-56.598z">
                                                        </path>
                                                    </svg><img src="/images/VipBlackjack.67b994b7.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">BCGAME VIP Blackjack</span>
                                                </a><a href="/game/pp23:424i" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="add-icon game-icon-svg icon"
                                                        viewBox="0 0 1024 1024">
                                                        <path
                                                            d="m155.809 388.588 367.879 525.394c20.534 29.33 63.233 34.987 95.186 12.614L862.8 755.787 693.259 997.962c-18.642 26.625-55.218 33.706-83.85 17.17l-4.964-3.164L63.536 633.22c-27.514-19.265-35.973-55.464-20.416-83.16l3.199-5.086 109.492-156.386zM618.307 25.03l.588.824 358.813 512.439c19.197 27.421 12.374 65.519-15.117 85.429l-.839.598-330.706 231.56c-27.909 19.542-66.129 13.392-85.706-13.604l-.588-.824-358.813-512.439c-19.197-27.421-12.374-65.513 15.117-85.424l.839-.598 330.706-231.56c27.907-19.54 66.128-13.396 85.706 13.6zm58.843 326.165c-16.538 2.702-31.631 8.834-45.042 18.222-20.317 14.227-33.984 34.191-40.199 56.079-22.694-1.644-46.127 4.369-66.445 18.593-13.411 9.39-24.334 21.479-32.533 36.096-16.78 30.044-17.609 66.462-2.155 97.423 16.313 32.527 47.503 53.997 83.465 57.501.211.029.453.102.718.117l.203-.001 140.513 9.763c8.907.583 17.761-2.155 24.909-7.157 7.072-4.952 12.56-12.007 15.085-20.412l.16-.55 38.883-135.376c.061-.256.148-.644.206-.902 9.086-35.049-.5-71.641-25.486-98.096l.004.001c-23.729-25.173-58.232-36.848-92.286-31.299zm-505.006 159.62c-10.76-3.095-21.203-3.176-31.33-.243s-19.243 8.453-27.35 16.56c-8.136 8.136-13.664 17.26-16.582 27.372s-2.823 20.54.287 31.285c3.11 10.745 9.647 21.1 19.611 31.064 9.935 9.935 20.267 16.45 30.998 19.545s21.166 3.199 31.307.31c10.141-2.889 19.28-8.402 27.416-16.538 8.107-8.107 13.627-17.224 16.56-27.35s2.852-20.569-.243-31.33c-3.095-10.76-9.625-21.122-19.589-31.086s-20.326-16.494-31.086-19.589zm-25.139 27.438c5.203-1.017 10.672-.324 16.405 2.078s11.549 6.552 17.445 12.448c5.896 5.896 10.045 11.711 12.448 17.445s3.095 11.202 2.078 16.405c-1.017 5.203-3.707 9.986-8.07 14.349-4.334 4.334-9.109 7.016-14.327 8.048s-10.694.346-16.428-2.056c-5.734-2.403-11.549-6.552-17.445-12.448s-10.045-11.711-12.448-17.445c-2.403-5.734-3.088-11.21-2.056-16.428s3.714-9.994 8.048-14.327c4.363-4.363 9.146-7.053 14.349-8.07zm122.263 33.433-28.064 10.059c-.365 9.243-3.107 17.877-7.99 25.96l31.813-10.453 4.242-25.567zM370.75 214.355l-39.793 26.841-6.339 90.277-1.843 1.243-39.87-59.109-33.786 22.789 94.287 139.787 33.786-22.789-25.552-37.882 2.005-26.179 71.871 31.466 40.407-27.255-102.649-43.379 7.474-95.809z">
                                                        </path>
                                                    </svg><img src="/images/SpeedBaccarat.4e36e147.png"
                                                        className="game-icon-img" alt="" /></div><span className="game-name">BC Speed
                                                            Baccarat</span>
                                                </a><a href="/game/pp23:902a13" keep-scroll-position="true" className="game-item">
                                                    <div className="game-icon"><svg className="add-icon game-icon-svg icon"
                                                        viewBox="0 0 1024 1024">
                                                        <path
                                                            d="m576.453 943.276-196.13 59.963c-34.024 10.403-56.359-1.19-66.996-34.768l-.319-1.024-7.39-24.172h270.835zm180.696-823.055c32.356 0 48.701 16.019 49.024 48.055l.005.975V855.66c0 32.356-16.021 48.701-48.054 49.024l-.975.005H266.857c-32.358 0-48.701-16.021-49.024-48.054l-.005-.975V169.251c0-32.358 16.019-48.701 48.054-49.024l.975-.005h490.292zm87.616 373.887 86.889 284.199c10.402 34.022-1.19 56.359-34.768 66.996l-1.024.319-51.22 15.661c.082-1.849.124-3.725.124-5.625l-.001-361.551zm-330.221-22.855h-5.073c-.687.506-1.401.989-2.121 1.466l-1.623 1.069c-1.8 1.194-3.548 2.433-4.933 3.994-29.218 32.882-59.815 64.376-93.198 93.08-8.886 7.64-18.314 14.673-26.926 22.601-17.646 16.244-27.606 36.485-29.772 60.458-.062.687-.272 1.361-.448 2.039l-.101.407v10.143c.163.996.316 1.994.464 2.992l.441 2.998c.443 2.998.91 5.99 1.584 8.935 8.305 36.318 38.624 64.575 75.447 70.613 18.286 2.998 35.795 1.445 51.954-8.397 3.398-2.073 6.566-4.53 10.082-6.98.086.86.231 1.632.234 2.401.012 5.282-.007 10.565-.011 15.848-.011 9.575-3.925 17.145-11.754 22.633a258.501 258.501 0 0 1-4.031 2.763l-4.057 2.723a217.103 217.103 0 0 0-6.022 4.172c-4.151 3.002-4.313 4.85-1.056 8.415l.2.216h95.726c3.697-3.108 3.743-5.473-.15-8.344-3.821-2.813-7.742-5.515-11.775-8.019-9.73-6.035-14.601-14.584-14.211-26.127.186-5.477.032-10.968.032-16.576.494.092.858.048 1.049.21 16.713 14.176 36.154 18.803 57.432 15.573 33.705-5.116 58.467-22.889 73.412-53.747 4.5-9.29 6.965-19.213 7.768-29.53.059-.775.273-1.537.452-2.303l.103-.46.004-.01v-10.143c-.959-5.594-1.594-11.276-2.948-16.775-5.018-20.359-15.736-37.289-32.099-50.333-41.944-33.426-79.623-71.224-115.178-111.247-1.571-1.77-3.583-3.144-5.617-4.5l-1.109-.739a51.832 51.832 0 0 1-2.172-1.521zM179.41 162.701a125.798 125.798 0 0 0-.171 6.549l-.001 360.661-86.894-284.217c-10.403-34.024 1.188-56.358 34.768-66.996l1.024-.319 51.271-15.677zm210.017 32.652h-39.601v101.318c-.048 3.8-.604 6.988-1.662 9.562s-2.613 4.51-4.661 5.809c-2.048 1.299-4.564 1.948-7.552 1.948-2.939 0-5.431-.637-7.479-1.912s-3.637-3.187-4.77-5.737c-1.066-2.399-1.693-5.311-1.884-8.734l-.031-.648h-40.18c-.047 11.354 2.265 20.579 6.938 27.674s10.936 12.292 18.789 15.587c7.853 3.295 16.525 4.943 26.015 4.943 10.936 0 20.608-2.032 29.014-6.098s15.007-9.742 19.8-17.03c4.667-7.097 7.085-15.299 7.253-24.608l.009-.757V195.352zM710.673 55.531l.319 1.024 7.667 25.078H444.575l199.1-60.871c34.022-10.402 56.358 1.188 66.996 34.768z">
                                                        </path>
                                                    </svg><img src="/images/OneBlackjack.7b0de011.png" className="game-icon-img"
                                                        alt="" /></div><span className="game-name">BC ONE BlackJack</span>
                                                </a></div>
                                        </div>
                                    </div>
                                </div>


                                <a href="/gamelist/slots" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Slots"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Slots</span></div>
                                </a><a href="/gamelist/live" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_LiveCasino"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Live Casino</span></div>
                                </a><a href="/tagname/hot" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_HotGame"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Hot Games</span></div>
                                </a><a href="/tagname/new-releases" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_NewReleases"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>New Releases</span></div>
                                </a><a href="/tagname/feature-buy-in" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_FeatureBuy-in"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Feature Buy-in</span></div>
                                </a><a href="/tagname/blackjack" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Blackjack"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Blackjack</span></div>
                                </a><a href="/tagname/table-games" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_TableGame"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Table Games</span></div>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div className={sports ? "o14ab7td nav-item-wrap navbox-open" : "o14ab7td nav-item-wrap"}>
                        <div className={sports ? "toggle-tigger nav-item open-tigger" : "toggle-tigger nav-item"} onClick={() => setSports(!sports)}>

                            <div className="nav-item-left">
                                <svg className="s1ff97qc icon">
                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Sports"></use>
                                </svg></div>
                            <div className="nav-item-right"><span>Sports</span><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Arrow"></use>
                            </svg></div>
                        </div>
                        <div className="v1qcofba" style={sports ? { height: 'auto' } : { height: '0px' }}>
                            <div className="sports-sub-navs"><a href="/sports?bt-path=%2Flive" keep-scroll-position="true"
                                className="nav-item">
                                <div className="nav-item-left"><svg className="s1ff97qc icon">
                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_LiveEvents"></use>
                                </svg></div>
                                <div className="nav-item-right"><span>Live Events</span></div>
                            </a><a href="/sports?bt-path=%2Fsoccer-1" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Soccer"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Soccer</span></div>
                                </a><a href="/sports?bt-path=%2Ftennis-5" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Tennis"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Tennis</span></div>
                                </a><a href="/sports?bt-path=%2Fbasketball-2" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Basketball"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Basketball</span></div>
                                </a><a href="/sports?bt-path=%2Fcricket-21" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Cricket"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Cricket</span></div>
                                </a><a href="/sports?bt-path=%2Ffifa-300" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_FIFA"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>FIFA</span></div>
                                </a><a href="/sports?bt-path=%2Famerican-football-16" keep-scroll-position="true"
                                    className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_AmericanFootball"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>American Football</span></div>
                                </a><a href="/sports?bt-path=%2Fice-hockey-4" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_IceHockey"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Ice Hockey</span></div>
                                </a><a href="/sports?bt-path=%2Fbaseball-3" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Baseball"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Baseball</span></div>
                                </a><a href="/sports?bt-path=%2Fhandball-6" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Handball"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Handball</span></div>
                                </a></div>
                        </div>
                    </div>
                    <div className="nav-item-wrap"><a href="/racing" keep-scroll-position="true" className="nav-item">
                        <div className="nav-item-left"><svg className="inpcsja" version="1.1" xmlns="http://www.w3.org/2000/svg"
                            width="30" height="32" viewBox="0 0 30 32">
                            <path
                                d="M23.707-0c-0.047 0-0.109 0-0.155 0.016-0.078 0.016-1.54 0.404-2.333 1.182s-1.213 1.804-1.369 2.582c-7.636 0.264-14.993 4.557-14.993 13.702 0 2.364 0.7 6.159 1.757 7.714 0 0 0.995 2.022 5.817 4.728 3.282 1.089 6.579 1.182 7.667 1.182 0.187 0 0.311 0 0.342 0 0.264 0 0.482-0.187 0.56-0.435 0.031-0.078 0.607-1.866 0.607-3.81 0-2.022-0.809-3.313-1.586-4.572-0.84-1.338-1.711-2.737-1.711-5.195 0-1.26 0.451-2.193 1.337-2.799 1.291 2.411 3.717 2.986 4.728 3.126 0.202 0.607 0.669 1.633 1.509 1.96 0.373 0.156 0.793 0.156 1.166 0 0.187 0.109 0.42 0.187 0.638 0.156 0.653-0.015 1.322-0.467 1.788-1.26 0.047-0.078 0.093-0.155 0.156-0.233 0.513-0.762 1.555-2.348-0.762-4.572-1.104-4.744-3.079-7.076-3.826-7.978l-0.093-0.14c-0.358-0.42-0.731-0.84-1.12-1.244l0.498-3.422c0.031-0.202-0.047-0.404-0.187-0.544-0.124-0.078-0.28-0.14-0.436-0.14zM19.415 0.124c-0.031 0-0.047 0.016-0.078 0.016-0.482 0.062-0.809 0.451-1.151 0.84-0.14 0.156-0.404 0.451-0.544 0.529-0.124-0.031-0.389-0.233-0.56-0.342-0.482-0.327-1.089-0.747-1.773-0.591-0.575 0.109-0.918 0.669-1.244 1.182-0.124 0.202-0.373 0.575-0.513 0.684-0.14-0.047-0.42-0.218-0.607-0.327-0.529-0.311-1.089-0.638-1.649-0.451-0.591 0.202-1.026 0.995-1.198 1.571-0.062 0.187-0.156 0.513-0.233 0.638-0.156-0.016-0.575-0.14-0.762-0.218-0.56-0.249-1.338-0.311-1.913-0.031-0.607 0.311-0.747 1.306-0.731 1.897 0 0.171 0 0.451-0.016 0.591s-0.062 0.171-0.109 0.202c-0.078 0.062-0.327 0.062-0.529 0.093-0.529 0.062-1.244-0.047-1.617 0.42-0.389 0.529-0.342 1.151-0.202 1.742 0.078 0.202 0.187 0.622 0.171 0.7-0.078 0.093-0.171 0.156-0.404 0.249-0.591 0.202-1.306 0.451-1.54 1.166-0.14 0.482-0.062 0.778 0.233 1.244 0.078 0.124 0.218 0.358 0.28 0.482-0.093 0.078-0.296 0.202-0.42 0.264-0.42 0.233-0.98 0.56-1.104 1.229-0.062 0.482 0.342 0.995 0.622 1.4 0.156 0.202 0.28 0.498 0.358 0.622-0.078 0.078-0.529 0.513-0.622 0.575-0.373 0.28-0.886 0.669-0.886 1.306 0 0.435 0.467 1.509 0.793 1.913 0.124 0.155-0.047 0.42-0.156 0.576-0.047 0.124-0.404 0.513-0.607 0.731-0.249 0.295-0.762 1.011-0.7 1.618 0.078 0.622 0.669 1.291 1.135 1.664 0.311 0.233 0.311 0.389 0.404 0.467 0.031 0.093 0.109 0.171 0.171 0.233 0.062 0.047 0.902 0.855 3.841 0.84-1.322-1.944-1.944-6.143-1.944-8.367 0-9.612 7.481-14.355 15.413-14.884 0.202-0.513 0.544-1.306 1.244-2.115-0.513-0.342-0.746-0.373-0.855-0.358zM25.496 1.182l-0.358 2.551c0.109 0.109 0.218 0.249 0.327 0.373 0.202-0.607 0.498-1.26 0.995-1.804 0.155-0.156 0.218-0.389 0.155-0.607s-0.249-0.373-0.467-0.435c-0.202-0.047-0.404-0.078-0.606-0.078h-0.047zM23.832 9.331c0.513 0 0.933 0.42 0.933 0.933s-0.42 0.933-0.933 0.933c-0.513 0-0.933-0.42-0.933-0.933s0.42-0.933 0.933-0.933z">
                            </path>
                        </svg></div>
                        <div className="nav-item-right"><span>Racing</span></div>
                    </a></div>
                    <div className="nav-item-wrap"><a href="/lottery" keep-scroll-position="true" className="nav-item">
                        <div className="nav-item-left"><svg className="s1ff97qc icon">
                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Lottery"></use>
                        </svg></div>
                        <div className="nav-item-right"><span>Lottery</span></div>
                    </a></div>
                    <div className="nav-item-wrap"><a href="/bingolist" keep-scroll-position="true" className="nav-item">
                        <div className="nav-item-left"><svg className="s1ff97qc icon">
                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Bingo2"></use>
                        </svg></div>
                        <div className="nav-item-right"><span>Bingo</span></div>
                    </a></div>
                    <div className="nav-item-wrap list-nav">
                        <a href="#/vip" keep-scroll-position="true" className="nav-item"
                        target="_self">
                        <div className="nav-item-left"><svg className="s1ff97qc icon">
                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_VipClub"></use>
                        </svg></div>
                        <div className="nav-item-right"><span className="v"><span className="y">VIP</span>Club</span></div>
                    </a>
                        <div className={exclusive ? "o14ab7td nav-item-wrap navbox-open" : "o14ab7td nav-item-wrap"}>
                            <div className={exclusive ? "toggle-tigger nav-item open-tigger" : "toggle-tigger nav-item"} onClick={() => setExclusive(!exclusive)}>
                                <div className="nav-item-left"><svg className="s1ff97qc icon">
                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Exclusive"></use>
                                </svg></div>
                                <div className="nav-item-right"><span>BC exclusive</span><svg className="s1ff97qc icon">
                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Arrow"></use>
                                </svg></div>
                            </div>
                            <div className="v1qcofba" style={exclusive ? { height: 'auto' } : { height: '0px' }}>
                                <div className="sports-sub-navs">
                                    <a href="/exclusive/daily-contest" keep-scroll-position="true"
                                    className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_DailyContest"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Daily Contest</span></div>
                                </a><a href="/promotion" keep-scroll-position="true" className="nav-item">
                                        <div className="nav-item-left"><svg className="s1ff97qc icon">
                                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Promotion"></use>
                                        </svg></div>
                                        <div className="nav-item-right"><span>Promotions</span></div>
                                    </a><a href="/exclusive/weekly-raffle" keep-scroll-position="true" className="nav-item">
                                        <div className="nav-item-left"><svg className="s1ff97qc icon">
                                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_WeeklyRaffle"></use>
                                        </svg></div>
                                        <div className="nav-item-right"><span>Weekly Raffle</span></div>
                                    </a></div>
                            </div>
                        </div><a href="/affiliate" keep-scroll-position="true" className="nav-item" target="_self">
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Affiliate"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Affiliate </span></div>
                        </a><a href="https://forum.bc.game/" keep-scroll-position="true" className="nav-item" target="_blank">
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Forum"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Forum </span><svg className="s1ff97qc icon icon-blank">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_NewWindow"></use>
                            </svg></div>
                        </a><a href="/help" keep-scroll-position="true" className="nav-item" target="_self">
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_ProvablyFair"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Provably Fair </span></div>
                        </a><a href="https://blog.bc.game/" keep-scroll-position="true" className="nav-item" target="_blank">
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Blog"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Blog </span><svg className="s1ff97qc icon icon-blank">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_NewWindow"></use>
                            </svg></div>
                        </a><a href="https://betting.bc.game/" keep-scroll-position="true" className="nav-item" target="_blank">
                            <div className="nav-item-left"><svg className="inpcsja" width="18" height="18" viewBox="0 0 1024 1024"
                                fill="none">
                                <path
                                    d="M473.297 744.974c16.507 0 29.89 13.383 29.89 29.89v95.825c0 16.507-13.383 29.89-29.89 29.89h-56.264c-16.507 0-29.89-13.383-29.89-29.89v-95.825c0-16.507 13.383-29.89 29.89-29.89h56.264zM314.17 672.888c16.507 0 29.89 13.383 29.89 29.89v167.919c0 16.507-13.383 29.89-29.89 29.89h-56.264c-16.507 0-29.89-13.383-29.89-29.89v-167.919c0-16.507 13.383-29.89 29.89-29.89h56.264zM498.198 123.413c160.864 0 291.287 130.426 291.287 291.287-0.085 69.375-24.42 133.070-64.931 183.087l42.368 42.368-0.959 0.93 0.366-0.308c9.144-7.258 22.477-6.642 30.948 1.801l147.504 147.504c9.088 9.088 9.088 23.86 0 32.969l-37.472 37.472c-9.088 9.088-23.86 9.088-32.969 0l-147.504-147.504c-8.44-8.44-9.059-21.773-1.801-30.948l-0.366 0.366-42.284-42.284c-50.189 41.076-114.306 65.748-184.185 65.829-160.864 0-291.287-130.426-291.287-291.287s130.426-291.287 291.287-291.287zM498.198 193.152c-122.348 0-221.549 99.2-221.549 221.549s99.2 221.549 221.549 221.549c122.348 0 221.549-99.2 221.549-221.549-0.109-122.321-99.224-221.435-221.549-221.549zM502.747 294.847c100.282 0 181.547 74.2 181.547 109.937s-81.265 109.851-181.547 109.851c-100.282 0-181.547-75.541-181.547-109.851s81.265-109.937 181.547-109.937zM502.661 323.773c-44.724 0-80.984 36.233-80.984 80.924 0 44.242 35.542 80.201 79.649 80.912l1.443 0.012c44.229 0 80.159-35.464 80.875-79.485l0.012-1.337v-0.102c0-44.693-36.261-80.924-80.984-80.924zM507.263 352.399c-2.745 4.323-4.384 9.432-4.507 14.903l-0.009 0.53-0.012 0.188 0.006 0.478c0.269 15.799 13.176 28.525 29.045 28.525 7.954 0 15.168-3.199 20.423-8.386l0.304-0.304-0.111-0.376c1.746 5.112 2.749 10.584 2.854 16.274l0.011 0.741-0.009 0.865c-0.464 28.573-23.789 51.597-52.497 51.597-28.999 0-52.506-23.491-52.506-52.468 0-28.688 23.037-51.995 51.639-52.459l0.866-0.009h4.439l0.077-0.111zM102.292 524.306h56.264c16.507 0 29.89 13.383 29.89 29.89v316.494c0 16.507-13.383 29.89-29.89 29.89h-56.264c-16.507 0-29.89-13.383-29.89-29.89v-316.494c0-16.507 13.383-29.89 29.89-29.89z">
                                </path>
                            </svg></div>
                            <div className="nav-item-right"><span>Sport Betting Insights </span><svg
                                className="s1ff97qc icon icon-blank">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_NewWindow"></use>
                            </svg></div>
                        </a>
                    </div>
                    <div className={sponsorship ? "o14ab7td nav-item-wrap navbox-open" : "o14ab7td nav-item-wrap"}>
                        <div className={sponsorship ? "toggle-tigger nav-item open-tigger" : "toggle-tigger nav-item"} onClick={() => setSponsorship(!sponsorship)}>
                            <div className="nav-item-left"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Sponsorship"></use>
                            </svg></div>
                            <div className="nav-item-right"><span>Sponsorships</span><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Arrow"></use>
                            </svg></div>
                        </div>
                        <div className="v1qcofba" style={sponsorship ? { height: 'auto' } : { height: '0px' }}>
                            <div className="sports-sub-navs"><a href="/sponsorship/afa" keep-scroll-position="true"
                                className="nav-item">
                                <div className="nav-item-left"><svg className="s1ff97qc icon">
                                    <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_AFA"></use>
                                </svg></div>
                                <div className="nav-item-right"><span>AFA</span></div>
                            </a><a href="/sponsorship/cloud-9" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Cloud9"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Cloud9</span></div>
                                </a><a href="/sponsorship/dl" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Dl"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>David Luiz</span></div>
                                </a><a href="/sponsorship/suniel" keep-scroll-position="true" className="nav-item">
                                    <div className="nav-item-left"><svg className="s1ff97qc icon">
                                        <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_SunielShetty"></use>
                                    </svg></div>
                                    <div className="nav-item-right"><span>Suniel Shetty</span></div>
                                </a></div>
                        </div>
                    </div>
                    <div className="nav-item-wrap support-nav"><button className="nav-item">
                        <div className="nav-item-left"><svg className="s1ff97qc icon">
                            <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Support"></use>
                        </svg></div>
                        <div className="nav-item-right"><span>Live Support</span></div>
                    </button></div>
                    <div className="theme-nav">
                        <div className="tftnvkv theme-wrap">
                            <div className="theme-inner"><button className="active"><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Dark"></use>
                            </svg><span>Dark</span></button><button className=""><svg className="s1ff97qc icon">
                                <use xlinkHref="/images/symbol-defs.ef6a79c4.svg#icon_Light"></use>
                            </svg><span>Light</span></button>
                                <div className="theme-bg"></div>
                            </div>
                        </div>
                    </div>
                </div> */}
            </div>
        </div >

    )
}

export default SideBar